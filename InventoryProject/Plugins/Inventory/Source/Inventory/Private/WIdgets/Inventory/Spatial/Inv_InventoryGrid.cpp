...
#include "Items/Fragments/Inv_FragmentsTag.h"
...
FInv_SlotAvailabilityResult UInv_InventoryGrid::HasRoomForItem(const FInv_ItemManifest& Manifest)
{
	...
	for (const auto& GridSlot : GridSlots)
	{
		...
		if (!HasRoomAtIndex(GridSlot, GetItemDimensions(Manifest), CheckedIndices, TentativeClaimed, Manifest.GetItemType(), MaxStackSize))
		{
			continue;
		}
...
bool UInv_InventoryGrid::HasRoomAtIndex(const UInv_GridSlot* GridSlot, const FIntPoint& Dimensions, const TSet<int32>& CheckedIndices,
	TSet<int32>& OutTentativeClaimed, const FGameplayTag& ItemType, const int32 MaxStackSize)
{
	bool bHasRoomAtIndex = true;

	 UInv_InventoryStatics::ForEach2D(GridSlots, GridSlot->GetIndex(), Dimensions, Columns, [&](const UInv_GridSlot* SubGridSlot)
	 {
		 if (CheckSlotConstraints(GridSlot, SubGridSlot, CheckedIndices, OutTentativeClaimed, ItemType, MaxStackSize))
		 {
			 OutTentativeClaimed.Add(SubGridSlot->GetIndex());
		 }
	 	else
	 	{
...
bool UInv_InventoryGrid::CheckSlotConstraints(const UInv_GridSlot* GridSlot, const UInv_GridSlot* SubGridSlot, const TSet<int32>& CheckedIndices,
	TSet<int32>& OutTentativeClaimed, const FGameplayTag& ItemType, const int32 MaxStackSize) const
{
	...
	if (!DoesItemMatch(SubItem, ItemType)) return false;
...
int32 UInv_InventoryGrid::DeterminFillAmountForSlot(const bool bStackable, const int32 MaxStackSize, const int32 AmountToFill,
...
void UInv_InventoryGrid::AddItem(UInv_InventoryItem* Item)
{
	...
	{
		UE_LOG(LogTemp, Warning, TEXT("Not Match!"));
		return;
	}
